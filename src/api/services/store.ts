...
export function createBot(ownerId: string, name: string, endpoint: string, authToken: string): Bot {
  const bot: Bot = {
    id: nanoid(),
    ownerId,
    name,
...
export function createTopic(text: string, category: string, proposerId: string): Topic {
  const topic: Topic = {
    id: nanoid(),
    text,
    category,
...
export function voteTopic(topicId: string, upvote: boolean): Topic | undefined {
