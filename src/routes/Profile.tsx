...
function AchievementCard({ achievement }: { achievement: Achievement }) {
  const icon = achievementIcons[achievement.icon as keyof typeof achievementIcons] || achievementIcons["default"];

  return (
    <div className="flex flex-col items-center p-4 rounded-lg bg-arena-border/30 hover:bg-arena-border/50 transition-colors">
      ...
      <div className="flex items-center gap-4">
        <div
          className={`text-sm font-medium ${
            match.won ? "text-arena-pro" : "text-arena-con"
          }`}
        >
          {match.won ? "+" : ""}
          {match.eloChange}
        </div>
        <Badge variant={match.won ? "pro" : "con"}>
          {match.won ? "Won" : "Lost"}
        </Badge>
      </div>
    </div>
...
export function ProfilePage() {
  ...
  const currentRankThreshold =
    user.rank === "champion" ? 3000 : user.rank === "diamond" ? 2500 : user.rank === "platinum" ? 2000 : user.rank === "gold" ? 1500 : user.rank === "silver" ? 1000 : 0;
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-gray-400 mb-4">{shortAddress}</p>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex justify-between text-sm text-gray-400 mt-2">
            ...
            <span className="text-arena-accent font-medium">{user.elo} ELO</span>
          ...
          <div className="space-y-3">
            {mockUserBots.map((bot) => {
              ...
              return (
                <div
                  key={bot.id}
                  className="flex items-center gap-4 p-3 rounded-lg bg-arena-border/30"
                >
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {/* Locked achievements */}
            <div className="flex flex-col items-center p-4 rounded-lg bg-arena-border/20 opacity-50">
